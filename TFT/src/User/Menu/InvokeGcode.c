...
void menuInvokeGcode(void)
{
  ...
  menuDrawSendGcode();
  while(infoMenu.menu[infoMenu.cur] == menuInvokeGcode)
  {
...
void sendGcodeTerminalCache(char *stream, TERMINAL_SRC src)
{
  ...
  if (infoMenu.menu[infoMenu.cur] != menuInvokeGcode && infoMenu.menu[infoMenu.cur] != menuTerminal) return;
